...
def test_capture_uses_actual_synthesizer_input_when_available(monkeypatch):
    """_build_capture_request uses round_executor._last_synthesizer_input if set."""
    ...
    fake_doc_analysis.model_dump.return_value = {"thesis": "Long EUR rates"}
    ...
    actual_synth_input = {
        "base": {"document": {"research_id": 7}},
        "forum_context": {"arguments": [{"argument_id": "a1"}], "relations": []},
    }
    round_executor = MagicMock()
    round_executor.run.return_value = fake_doc_analysis
    round_executor._last_synthesizer_input = actual_synth_input
    ...
    assert captured[0].input_payload == actual_synth_input
