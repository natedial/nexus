...
Gold argument maps with claim roles and links between claims live in `evals/golden/arguments.jsonl`. They are written only through `python -m research_analysis_layer.evals.gold_set`, usually while co-reading a document with the user. Follow [`.cursor/skills/gold-argument-annotation/SKILL.md`](../../.cursor/skills/gold-argument-annotation/SKILL.md).
